...
//send a custom message by saying someone user name or saying a specific word (can be done multiple time, consult Examples.js)
  if (message.startsWith(`I love you more`)) {
   bot.message.send(`Shush! I love you more 👁👄👁🔪 `);
  };

  if (message.startsWith(`726`)) {
   bot.message.send(`Give Me Your Office 😏`);
  };
...
bot.on('playerJoin', (user) => {
  ...
  bot.message.send(`Welcome to HR Disneyland! Go ride the Roller Toaster! 🦈🦈🦈 @${user.username} !`);
});
